...
namespace Farseer2Json
{
    ...
    //}
        ...
        public JObject B2n(b2World world)
		{
            ...
            b2Joint tmpJoint = world.GetJointList();
            while (true)
            {
                if (tmpJoint.GetType() == b2JointType.e_gearJoint )
                     continue;
        ...
        public JObject B2n(b2Fixture fixture)
		{
            ...
            if ((int)fixture.GetFilterData().categoryBits != 0x0001)
                fixtureValue["filter-categoryBits"] = (int)fixture.GetFilterData().categoryBits;
            if ((int)fixture.GetFilterData().maskBits != 0xffff)
                fixtureValue["filter-maskBits"] = (int)fixture.GetFilterData().maskBits;
            if (fixture.GetFilterData().groupIndex != 0)
                fixtureValue["filter-groupIndex"] = fixture.GetFilterData().groupIndex;
                ...
                case b2Shape.Type.e_edge:
			{
                ...
                if (chain.PrevVertex != null && chain.PrevVertex != zero)
				{
					shapeValue["hasPrevVertex"] = true;
					VecToJson("prevVertex", chain.PrevVertex, shapeValue);
				}
                if (chain.NextVertex != null && chain.NextVertex != zero)
				{
                ...
                case b2Shape.Type.e_polygon:
			{
				...
				break;
			default:
				Console.WriteLine("Unknown shape type : " + shape.GetType());
				break;
			}
        ...
        b2Fixture j2b2Fixture(b2Body body, JObject fixtureValue)
		{
			...
			else if (null != fixtureValue["edge"])
			{
			    ...
			    edgeShape.m_hasVertex0  = edgeValue["hasVertex0"] == null ? false : (bool)edgeValue["hasVertex0"];
                edgeShape.m_hasVertex3 = edgeValue["hasVertex3"] == null ? false : (bool)edgeValue["hasVertex3"];

                if (edgeShape.m_hasVertex0)
                    edgeShape.m_vertex0 = jsonToVec("vertex0", edgeValue);
                if (edgeShape.m_hasVertex3)
				    edgeShape.m_vertex3 = jsonToVec("vertex3", edgeValue);
			...
			else if (null != fixtureValue["chain"])
			{
                ...
                chainShape.m_hasPrevVertex = chainValue["hasPrevVertex"] == null ? false : (bool)chainValue["hasPrevVertex"];
				chainShape.m_hasNextVertex = chainValue["hasNextVertex"] == null ? false : (bool)chainValue["hasNextVertex"];

                if (chainShape.m_hasPrevVertex)
					chainShape.m_prevVertex = (jsonToVec("prevVertex", chainValue));
                if (chainShape.m_hasNextVertex)
					chainShape.m_nextVertex = (jsonToVec("nextVertex", chainValue));
